...
2. Apply Prisma migrations against Azure SQL (`pnpm --filter api run prisma:migrate`).
3. Update the App Service or Container App to use the new image. Enable the managed identity and give it access to Key Vault secrets if you are using Key Vault references.
4. Populate the configuration values above in App Service (or bind Key Vault references) and set `NODE_ENV=production`.
5. Validate health by hitting `/healthz` (or the root endpoint) and running smoke tests.
6. Monitor Application Insights for errors during the warm-up window and roll back if required.
