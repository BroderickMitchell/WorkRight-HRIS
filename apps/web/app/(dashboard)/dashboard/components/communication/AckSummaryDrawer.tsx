...
export default function AckSummaryDrawer({
  open,
  onOpenChange,
  loading,
  summary,
  post,
}: AckSummaryDrawerProps) {
  const acknowledged =
    summary?.recipients.filter((recipient) => recipient.acknowledged) ?? [];
  const pending =
    summary?.recipients.filter((recipient) => !recipient.acknowledged) ?? [];
  ...
  const exportCsv = () => {
    ...
    summary.recipients.forEach((recipient) => {
      const teams = recipient.teamIds.length
        ? recipient.teamIds
        : fallbackTeams;
      const name =
        `${recipient.user.givenName} ${recipient.user.familyName}`.trim();
  ...
  return (
    <Drawer open={open} onClose={handleClose} title="Acknowledgement progress">
      <div className="space-y-6">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            {post.ackSummary?.acknowledged ?? 0} of{' '}
            {post.ackSummary?.required ?? 0} recipients have acknowledged this
            update.
          </p>
        </div>
        ...
        {summary ? (
                ...
                {summary.perTeam.map(({ team, counts }) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <span className="font-medium text-foreground">
                        {teamLookup.get(team.id) ?? team.name}
                      </span>
                    ...
                    <div className="grid grid-cols-3 text-sm font-medium">
                      ...
                      <span>
                        {counts.required
                          ? Math.round(
                              (counts.acknowledged / counts.required) * 100,
                            )
                          : 0}
                        %
                      </span>
          ...
          <p className="text-sm text-muted-foreground">
            Acknowledgement data will appear once recipients are added.
          </p>
        )}
      </div>
    </Drawer>
