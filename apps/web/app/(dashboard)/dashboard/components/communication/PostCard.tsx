...
export default function PostCard({
  ...
  const editedLabel = post.editedAt
    ...
    <Card className="border-border/80">
      <CardHeader className="space-y-3">
            ...
            {post.requireAck ? (
              post.myAck?.acknowledged ? (
                <Badge className="flex items-center gap-1 text-xs">
                  ...
                  {post.myAck.acknowledgedAt
                    ? `on ${formatDateTime(post.myAck.acknowledgedAt)}`
                    : ''}
                </Badge>
              ) : (
        ...
        <div className="flex flex-wrap gap-2 text-xs">
          ...
          {post.requireAck ? (
            <Badge className="flex items-center gap-1 border-warning/40 bg-warning/10 text-warning">
      ...
      <CardContent className="space-y-4 text-sm leading-relaxed">
        ...
        {post.requireAck && canShowAckSummary ? (
          <div className="flex flex-wrap items-center gap-3 rounded-lg border border-border/70 bg-muted/40 p-3 text-xs">
            {post.ackSummary ? (
              <span>
                Acknowledged {post.ackSummary.acknowledged} of{' '}
                {post.ackSummary.required} &bull; Pending{' '}
                {post.ackSummary.pending}
              </span>
            ) : null}
            <Button
              size="sm"
              variant="secondary"
              onClick={() => setShowAckSummary(true)}
            >
