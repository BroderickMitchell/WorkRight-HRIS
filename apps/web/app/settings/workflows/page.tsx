...
interface WorkflowResources {
  taskTemplates: Array<{ id: string; name: string; defaultDueRules: any }>;
  ...
  surveys: Array<{ id: string; name: string; enabledCollectors: any }>;
...
function nodeColor(type: WorkflowNodeType, settings: Record<string, any>): string {
  if (type === 'condition') return 'bg-slate-600';
  if (type === 'dummy_task') return 'bg-purple-600';
  if (type === 'email' && settings?.schedule) return 'bg-orange-500';
  if (type === 'email') return 'bg-sky-500';
  if (type === 'course') return 'bg-emerald-500';
  return 'bg-blue-600';
}

function formatAssignment(settings: Record<string, any>, resources: WorkflowResources | null): string {
...
export default function WorkflowWorkbenchesPage() {
  ...
  const [nodes, setNodes, onNodesChange] = useNodesState([] as Node<WorkflowNodeData>[]);
  ...
  const updateNodeData = useCallback(
    (id: string, updater: (data: WorkflowNodeData) => WorkflowNodeData) => {
      setNodes((existing) =>
        existing.map((node) => (node.id === id ? { ...node, data: updater(node.data) } : node))
      );
    },
    [setNodes]
  );
      ...
      <header className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-600">
            Design conditional onboarding journeys with tasks, forms, courses, and notifications.
          </p>
        </div>
      </header>
...
function NodeConfiguration({ node, resources, edges, updateNodeData, updateEdgeLabel, removeNode }: NodeConfigurationProps) {
  ...
  const updateSettings = useCallback(
    (updater: (settings: Record<string, any>) => Record<string, any>) => {
      updateNodeData(node.id, (existing) => ({ ...existing, settings: updater(existing.settings ?? {}) }));
    },
...
interface AssignmentConfigProps {
  ...
  resources: WorkflowResources | null;
  onChange: (assignment: Record<string, any>) => void;
...
function AssignmentConfig({ assignment, resources, onChange, allowedModes }: AssignmentConfigProps) {
  ...
  const currentMode = (assignment?.mode ?? assignment) as AssignmentMode | undefined;
      ...
      <SelectInput
        ...
        onChange={(event) => {
          ...
          if (value === 'user' || value === 'group') {
            onChange({ mode: value, id: assignment?.id ?? null });
          } else {
            onChange({ mode: value });
          }
        }}
...
interface DueRuleProps {
  dueRule: Record<string, any> | null;
  onChange: (rule: Record<string, any> | null) => void;
}

function DueRuleConfig({ dueRule, onChange }: DueRuleProps) {
  const basis = dueRule?.basis ?? 'assignee.start_date';
  const direction = dueRule?.direction ?? 'AFTER';
  const offsetValue = dueRule?.offset?.value ?? 0;
  const offsetUnit = dueRule?.offset?.unit ?? 'days';
  const currentRule = {
    basis,
    direction,
    ...
    <div className="space-y-2">
      ...
      <div className="grid grid-cols-[1fr_auto] gap-2">
        ...
        <SelectInput
          value={offsetUnit}
          onChange={(event) =>
            onChange({ ...currentRule, offset: { value: offsetValue, unit: event.target.value } })
          }
        >
      ...
      <SelectInput
        value={direction}
        onChange={(event) =>
          onChange({ ...currentRule, direction: event.target.value })
        }
      >
            ...
            onChange={(event) =>
              updateSettings((prev) => ({
                ...
                schedule: {
                  relativeTo,
                  direction,
                  offset: { value: offsetValue, unit: event.target.value },
                  sendTime
                }
              }))
            }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        {edges.length === 0 && <p className="text-xs text-slate-500">Connect this node to define true/false branches.</p>}
      </div>
    </div>
  );
}
